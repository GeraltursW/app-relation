...
class CanonicalPage(Base):
    ...
    page_hash_id: Mapped[str | None] = mapped_column(String(64), nullable=True, unique=True, index=True)
...
class PageInstance(Base):
    ...
    ai_inference: Mapped[dict] = mapped_column(JSONB, default=dict)
    ai_recursive: Mapped[bool] = mapped_column(Boolean, default=False)
